...
# set doit=0 for all observations to skip the test
if ( any( doit!=0)) {
  # test
...
# corep has been set by function sct to the observation error variance
  ...
  if (argv$debug) 
    save.image(file.path(argv$debug.dir,"dqcres_sct.RData")) 
# case of no observation passed to SCT (skip SCT)
} else{
  rm( doit, eps2.sct, T2vec, T2posvec, T2negvec)
  if (!any(is.na(argv$const.corep))) {
    for (f in 1:nfin) {
      if (any(data$prid[ix]==argv$prid[f])) {
        ip<-which(data$prid[ix]==argv$prid[f])
    ...
    corep[] <- argv$mean.corep 
  } 
  if (argv$verbose | argv$debug) {
    print("no observations to check (doit set to 0 for all providers)")
    print("+---------------------------------+")
  }
}
#-----------------------------------------------------------------------------
